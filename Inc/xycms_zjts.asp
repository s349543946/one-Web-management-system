...
newsfl.open "select * from zjts_subdir order by pl_id asc",conn,1,1
if newsfl.eof and newsfl.bof then
response.Write("&nbsp;暂无信息记录 !")
end if
response.Write("<li><a href=""vip.asp?id="&newsfl("id")&""" title=""" & newsfl("title") &""">"  & newsfl("title") & "</a> </li>")
set newsfl=nothing
end function

function zjtslist02()
set newsfl=server.CreateObject("adodb.recordset")
newsfl.open "select * from zjts_subdir where pl_id = 2",conn,1,1
if newsfl.eof and newsfl.bof then
response.Write("&nbsp;暂无信息记录 !")
end if
